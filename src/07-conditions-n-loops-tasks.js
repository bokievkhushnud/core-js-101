...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (x1 > rect2.left && y1 > rect2.top && x2 > rect1.left && y2 > rect1.top) {
    return true;
  }
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const openB = ['(', '['];
  const closeB = [')', ']'];
  return `${openB[Number(isStartIncluded)]}${c}, ${d}${closeB[Number(isEndIncluded)]}`;
}
...
function reverseInteger(num) {
  return Number.parseInt(Array.from(num.toString()).reverse().join(''), 10);
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = ccnDigits.length - 1; i >= 0; i -= 1) {
...
function getCommonDirectoryPath(pathes) {
  ...
  const filenames = pathes.map((path) => path.split('/'));
  const firstFilename = filenames[0];

  let commonPath = '';
  for (let i = 0; i < firstFilename.length; i += 1) {
    const directory = firstFilename[i];

    if (!filenames.every((filename) => filename[i] === directory)) {
      break;
    }
